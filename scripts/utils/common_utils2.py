...
class CommonUtils:
    ...
    @staticmethod
    def get_image_idx(pixel_local, image_shape):
    ...
    def end_with_time(msg, start_time):
        print(msg, " Time cost: ", (datetime.datetime.now() - start_time))
    ...
    def get_delta(kw):
        ...
        #     return kw['delta']
            ...
            raise Exception("No such delta type ", delta_type)
...
class BaseScheduler(object):
    ...
    def check_time(self):
        if datetime.datetime.now() - self.start_time > self.time_limitation:
            print("FINAL RESULTS: #", self.name, "# Fail to find NaN! Iteration: ", self.iter_count, ". Time cost: ", self.time_limitation)
    ...
    #         print("# ", at, ": ", self.__getattribute__(at))
