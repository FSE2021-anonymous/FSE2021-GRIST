...
from scripts.utils.common_utils2 import CommonUtils,BaseScheduler


class TensorFlowScheduler(BaseScheduler):
    def __init__(self,name,drop_rate=None,change=None,switch_data=None):
        super(TensorFlowScheduler, self).__init__(name,drop_rate=drop_rate,change=change,switch_data=switch_data)

    def loss_checker(self, loss_val,log_msg=None):
        if log_msg is None:
            if self.iter_count % 100 == 0:
                print("Iteration:", self.iter_count, "Current Time cost:", datetime.now() - self.start_time, "Loss:", loss_val)
        if np.isnan(loss_val).any() or np.isinf(loss_val).any():
            if log_msg is not None:
                print(log_msg)
            s = "FINAL RESULTS: #" + self.name + "# Success to find NaN!  Iteration: " + self.iter_count + self.start_time
            CommonUtils.end_with_time()
...
class GradientSearcher(TensorFlowScheduler):
    ...
    def __init__(self, name,score_mthd=1, update_stragety='rate', drop_rate=None,change=None,switch_data=None):
        super(GradientSearcher, self).__init__(name,drop_rate=drop_rate,change=change,switch_data=switch_data)
    ...
    @staticmethod
    def iterative_grist(w, dw, pixle_boundary, update_stragety, update_list, clip_list, batch_size, useful_update_list, change, delta, delta_type, switch_data):
    ...
    def update_batch_data(self,session,monitor_var,feed_dict,input_data,):
        ...
        if 'delta' in monitor_var.keys():
            loss_val, obj_function_val, obj_grads_val,delta_val = session.run([monitor_var['loss'],
                                                                               monitor_var['obj_function'],
                                                                               monitor_var['obj_grad'],
                                                                               monitor_var['delta']],
                                                                              feed_dict=feed_dict)
        else:
            loss_val, obj_function_val, obj_grads_val = session.run(
                ...
                feed_dict=feed_dict)
            delta_val = None
        log_msg = "INFO: Iter:" + self.iter_count + "Current Time cost:" + (datetime.now() - self.start_time) + " Loss: "\
                  + loss_val + " Object function value: " + obj_function_val + " Change step: " + decay_change
        if self.iter_count % 100 == 0:
            print(log_msg)
    ...
    def switch_new_data(self,new_data_dict,old_data_dict,scores_rank):
        # change batch data
