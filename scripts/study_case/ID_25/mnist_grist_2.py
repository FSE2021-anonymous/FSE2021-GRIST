...
cross_entropy = tf.reduce_mean(-tf.reduce_sum(y_ * tf.sqrt(y), reduction_indices=[1]))
...
gradient_search = GradientSearcher(name="tensorflow_mnist_grist_2")
obj_function = tf.reduce_min(y)
