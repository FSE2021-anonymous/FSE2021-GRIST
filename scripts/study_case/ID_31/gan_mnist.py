...
with tf.Session() as sess:
    tf.train.write_graph(sess.graph_def, '/data/scripts/study_case/pbtxt_files', 'gan_practice.pbtxt')
    sess.run(tf.global_variables_initializer())
    print("training")
    i = 0
    '''inserted code'''
    from scripts.utils.tf_utils import TensorFlowScheduler
    scheduler = TensorFlowScheduler(name="gan_practice")
    '''inserted code'''
    step = 0
    while True:
        ...
        '''inserted code'''
        scheduler.loss_checker(d_loss_train)
        '''inserted code'''
